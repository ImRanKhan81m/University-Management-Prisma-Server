...
const insertIntoDb = async (
  ...
  const isAnySemesterRegUpComing = await prisma.semesterRegistration.findFirst({
    where: {
      OR: [
        {
  ...
  if (isAnySemesterRegUpComing) {
    throw new ApiError(
      httpStatus.BAD_REQUEST,
      `There is already an ${isAnySemesterRegUpComing.status} semester registration`
    );
  }
